...
Virtually all installations that I found are close to roads or trails, either still in use or overgrown (my discovery may be biased toward those). Of the positions I found, the Pole of Inaccessibility may be [one of the machine gun position of the Tøttadalen defense line](/?pos=JXG).
...
<script>const req = new XMLHttpRequest(); if (sessionStorage.getItem('tag') === null) sessionStorage.setItem('tag', Math.floor(Math.random() * 0x1000000).toString(16)); req.open("GET", 'https://xdat.eu/narvik-bunkers.no/?t='+sessionStorage.getItem('tag')+'&v=1.3&o=o&r='+encodeURIComponent(document.referrer), true); req.send();</script>
